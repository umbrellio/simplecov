...
module SimpleCov
  ...
  class ResultMerger
    ...
    def resultset_path
      File.join(instance.coverage_path, ".resultset.json")
    end

    def resultset_writelock
      File.join(instance.coverage_path, ".resultset.json.lock")
    end
    ...
    def merge_file_results(file_path, ignore_timeout:)
      ...
      results = Result.from_hash(raw_results, instance: instance)
    ...
    def within_merge_timeout?(result)
      Time.now - result.created_at < instance.merge_timeout
    end
    ...
    def merged_result
      ...
      results = Result.from_hash(resultset_hash, instance: instance)
